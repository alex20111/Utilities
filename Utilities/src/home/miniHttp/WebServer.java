...
//	private ServerConfig serverConfig = new ServerConfig();
	...
	public void addHandler(String path ,HttpHandler handler) throws IOException{
		addHandler(path, handler, null);
	} 
	public void addHandler(String path, HttpHandler handler, String externalPageName) throws IOException {
		if ( !path.startsWith("/")){
		...
		if (externalPageName != null && externalPageName.trim().length() > 0) {
			handler.setExternalPage(externalPageName);
		}
...
//		File fi = new File(WebServer.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath());
	...
	public Response serve(IHTTPSession session) {
		...
		try{			
			...
			System.out.println("Handler size: " + handlers + " Can serve: " + serve);
			if (serve) {
